...
export async function completeAmoTasks(ids: number[], resultText: string): Promise<boolean> {
  if (!ids.length) return true;
  const res = await amoPatch<unknown>(`/api/v4/tasks`, ids.map((id) => ({ id, is_completed: true, result: { text: resultText } })));
  return res !== null;
}
