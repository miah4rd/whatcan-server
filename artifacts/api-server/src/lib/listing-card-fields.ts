...
- counterpart: THE TEST IS NOT THEIR JOB TITLE. "Villa manager" is said both by a salaried employee of the owner and by a management company — the same words, opposite answers. What separates them is whether the person is ON THE OWNER'S PAYROLL or is a SEPARATE BUSINESS working this villa under its own contract, with its own brand and its own commission. The second one is another agency exactly like us.
  ...
  "manager" — a management COMPANY or resort operator that has this villa under contract: a separate legal entity with its OWN BRAND, taking its OWN cut. The tells: they introduce a company name ("Luxso Villa & Resort Management", "Desa Group"), speak as "we"/"kami" about that organisation, mention their office, admin, reservations team, rate contract or the commission they offer agents. They are on the villa's side, but as a business we share the fee with.
